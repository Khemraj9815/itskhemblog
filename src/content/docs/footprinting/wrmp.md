---
title: Windows Remote Management Protocol
description: wrmp
---

### Server Manager

- Centralized tool for managing multiple servers from one interface
- **Default setting**: Enabled by default from Windows Server 2016 onward

### **WS-Management Protocol (WinRM)**

- Allows secure remote management via HTTP/HTTPS
- Supports cross-platform server management and PowerShell remoting

### **Baseboard Management Controllers (BMC)**

- Provides hardware diagnostics and control (e.g., power management)
- Useful for managing hardware-level functions remotely

### **COM API and Script Objects**

- Enables automation with custom scripts and applications
- Useful for scaling tasks across many servers.

these tools provide a secure, efficiently manage server remotely. help administration, reduce physical visit to server, automate routine time

- Remote Desktop Protocol(RDP)
- Windows Remote Management(WinRm)
- Windows Management Instrumentation(WMI)

![image.png](https://prod-files-secure.s3.us-west-2.amazonaws.com/3af77eaa-2067-4012-8f99-416adebc73a4/363a2c23-c345-4717-abbf-946e2626ae3a/image.png)

# **RDP (**[Remote Desktop Protocol](https://docs.microsoft.com/en-us/troubleshoot/windows-server/remote/understanding-remote-desktop-protocol))

- protocol developed by Microsoft for remote access to windows OS
- allows to see GUI of remote system and control it
- when we connect via RDP  data are encrypted while transmitting
- RDP works at application layer
    - like transport and internet layer (data transfer)
- RDP usually uses TCP port 3389,
- RDP can use UDP **on port 3389**
- UDP is connection-less
- UDP is fast, but less reliable than TCP

![image.png](../../../assets/footprinting/udptcp.png)

- For RDP session, both the network and firewall should allow incoming connection on RDP port
- Public ip required
- configuration of port forwarding on the NAT router to redirect RDP traffic
- RDP use TLS/SSL to encrypt data and login data for security
- Default RDP certificates are self-signed, so clients can’t verify their authenticity, often triggering security warnings
- Remote Desktop on Windows Servers is pre-installed, can be activated in server manager
- By default, RDP only allows connections from hosts using NLA

# **Footprinting the Service**

- scanning RDP gives lot of information of host
- we can determine if NLA is enabled, the product version and the hostname
...
- we can use —packet-trace to track individual package and inspect manually
...
- Microsoft's implementation and also an extension of the Common Information Model (CIM)
